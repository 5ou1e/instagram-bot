...
def execute(
    ...
    new_arguments = [
        (
            execute(argument, cache)
            if isinstance(argument, list) and len(arguments)
            else argument
        )
...
@operand(
    "bk.action.caa.login.GetUniqueDeviceId",
...
def bloksgetscript(*args, data: dict, **kwargs):
    assert (
        data is not None
    ), 'data must be specified to execute "bk.action.bloks.GetScript" command'
...
def bloksgetvariable(*args, data: dict, **kwargs):
    assert (
        data is not None
    ), 'data must be specified to execute "bk.action.bloks.GetVariable2" command'
    var_id = args[0]
