...
class ConvertableBranch:
        ...
        if recursive is False:
            ...
            for key, value in content.items():
                if is_branch(value):
                    ...
                    value = ConvertableBranchList(
                        [ConvertableBranch(item, recursive=recursive) for item in value]
                    )
    ...
    def _opening_separator(self, as_hex: bool = False) -> str:
        attrs = [
            f"""{("x" + name.encode().hex().upper()) if as_hex else name}='{value}'"""
            for name, value in self.content.items()
            if isinstance(value, (ConvertableBranch, ConvertableBranchList)) is False
        ]
        return "<%s%s>" % (
            ("x" + self.name.encode().hex().upper() if as_hex else self.name),
            (" " + attrs_ if (attrs_ := " ".join(attrs)) else ""),
        )
    ...
    def _dumps(self, indent: int, level: int, as_hex: bool = False) -> str:
        ...
        for x, (name, value) in enumerate(self.content.items()):
            ...
            if isinstance(value, ConvertableBranchList) is True:
                data = [
                    branch.dumps(indent=indent, as_hex=as_hex, level=level + 1)
                    for branch in value
                ]
                if indent > 0:
...
def bloks_to_html(tree: dict, indent: int = 0, as_hex: bool = False) -> str:
    ...
    assert is_branch(
        tree
    ), 'tree is not a branch, make sure you are placing the value at ["layout"]["bloks_payload"]["tree"]'
