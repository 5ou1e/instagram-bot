...
from ..utils import get_branch_name, is_branch, is_branch_list


class MappingCache(list[BlokObjectBuilder]):
    pass
...
def recursion_mapping(
    ...
    for bkid, class_name in zip(blok_ids, class_names):
        datas.append(
            [
                (key, value)
                for key, value in branches[bkid][class_name].items()
