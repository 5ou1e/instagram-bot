...
def is_branch(__d: dict, /) -> bool:
    ...
    """
    return (isinstance(__d, dict) and len(__d) == 1) or (
        isinstance(__d, tuple) and len(__d) == 2 and isinstance(__d[1], dict)
    )
    ...
    def wrapper(self: BlokField, *args: Any, **kwargs: Any):
        ...
        if hasattr(self, "skip_type_verif") and hasattr(self, "accepted_types"):
            if self.skip_type_verif is False:
                check_type = (
                    lambda item: isinstance(item, self.accepted_types)
                    or item is Undefined
                )
                if isinstance(result, list) and self.is_list:
                    ...
                    raise TypeError(
                        f'the result of "{func.__name__}" is of type '
...
def hex_to_rgb(hex_color: str) -> tuple[int]:
    ...
    return tuple(
        int(hex_color[i : i + length // 3], 16) for i in range(0, length, length // 3)
    )
