...
from typing import Any, Callable, Self, Union, get_args, get_origin
...
class BlokField(MultipleAliasField):
    def __init__(
        ...
        if default == Undefined and default_factory is None:
            ...
            if default != Undefined:
                raise ValueError(
                    "`default` and `default_factory` arguments "
    ...
    def __repr__(self) -> str:
        return (
            f"BlokField(required: {self.required}, "
    ...
    def __to_py_obj__(self, content: Any, bkid: str, *args, **kwargs) -> Self:
        if is_branch_list(content):
            return [
                self.__to_py_obj__(
                    content=item, bkid=bkid, *args, verified=True, **kwargs
                )
                for item in content
            ]
        if (branch_name := get_branch_name(content)) is not None:
            sub_branches = {
                item.aliases[bkid]: item
                for item in self.accepted_types
                if issubclass(item, Branch)
            }
            if (matching_btype := sub_branches.get(branch_name)) is not None:
                return matching_btype.__to_py_obj__(
                    content=content, bkid=bkid, verified=True
                )
        else:
            for blok_field_type in [
                item for item in self.accepted_types if isinstance(item, BlokFieldType)
            ]:
                if blok_field_type.test_for(content) is True:
                    return blok_field_type.__to_py_obj__(
                        content=content, bkid=bkid, *args, verified=True, **kwargs
                    )
    ...
    def accepted_types(self, accepted_types: list[Self | type] | None) -> None:
        if accepted_types is not None:
            ...
            if (
                isinstance(accepted_types, GenericAlias)
                and accepted_types.__name__ == "list"
            ):
            ...
            if (
                all([item == Self or isinstance(item, type) for item in accepted_types])
                is False
            ):
                raise TypeError(
                    f"a value of `accepted_types` is not a `type`: {accepted_types}"
                )
...
class Branch(MultipleAliasField):
        ...
        **kwargs,
        ...
        if verified is False and branch_name is None:
            raise InvalidRawBranchError(
                "invalid branch sent to class " f"`{self.__class__.__name__}`"
            )
        ...
        for field_name, field_types in raw_fields.items():
            ...
            if isinstance(field_infos, BlokField) is False:
                raise TypeError(
                    "every field values must be instances of "
    ...
    def __setattr__(
        self, field_name: str, field_value: Any, bkid: str = None, *args, **kwargs
    ) -> None:
        if hasattr(self, "fields") and field_name in self.fields:
            ...
            if field_value is Undefined:
                ...
                field_value = field_infos.__to_py_obj__(
                    content=field_value,
                    bkid=bkid,
                    *args,
                    **kwargs,
    ...
    def __to_py_obj__(
        ...
        **kwargs,
        ...
        return cls(content=content, bkid=bkid, *args, verified=verified, **kwargs)
