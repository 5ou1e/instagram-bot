...
from src.domain.working_group.services.flows.auth.authorization_flow import (
    AuthorizationFlow,
...
class AccountWorkerAuthorizeAccountTaskExecutor(AccountWorkerTaskExecutor):
    ...
    async def execute(self, worker: AccountWorker, stop_event: asyncio.Event):
        ...
        async with self._uow:
            task = await self._account_worker_task_repository.get_by_id(
                self._subtask_id
            )
            working_group: WorkingGroup = (
                await self._working_group_repository.get_by_id(task.working_group_id)
            )
